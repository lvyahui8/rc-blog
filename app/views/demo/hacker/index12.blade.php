...
<script>
    ...
    var Hacker  = function(canvas,width,height){
        canvas.width = width;
        ...
        for(var i = 0,n = Math.floor(width / fw);i<n;i++){
            var nu = Math.floor(Math.random() * 4 + 3),
                    runWay = new RunWay(i * fw,[],Math.floor(Math.random() * 4 + 1));
            var headY = randY(height);

            for(var j = 0;j < nu;j++){
                var str = randomStr(Math.random() * 12);
